...
&if $MACH in [ SUN ] || $OSSPEC == SOLARIS
	print OUT "#pragma ident \"$identstring\\n\"\n";
&elif $OS in [ WIN32 ]
$identstring =~ s /\\/\\\\/g;  
if ($ENV{BIT64} eq "1")
{	print OUT "#pragma comment(exestr, \"$identstring\")\n" ; }
else
...
&if $OS in [ WIN32 ]
  return $return_val;
  open STDERR, ">&STDOUT" or die "can't duplicate STDERR\n";	
  select STDERR; $|=1; 
  select STDOUT; $|=1;
  ...
  {
    $return_val = <MS_CL>;
 print ("Return 1: $return_val\n");
   $return_val = <MSYSTDERR>;
 print ("Return 2: $return_val\n");   
    chop ( $return_val );
    close MS_CL;
  }
  else
  {
  	print "ERROR Can't get version from Micrsoft compiler ! \n$!\n";
  }
&else
