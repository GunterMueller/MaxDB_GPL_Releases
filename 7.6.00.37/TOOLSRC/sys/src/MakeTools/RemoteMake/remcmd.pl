...
my $remcommand = join " ", @ARGV;

$remcommand = SDB::Remote::WebMake::convert_to_httpstring($remcommand);

unless ($ENV{APOMAKE})
{
	@ARGV=( 'cmd', 'free');
	push @ARGV, @remcall_options;
	push @ARGV, "$platform", $remcommand  ;
...
{
	@ARGV = ( "-action=cmd", $platform );
	push @ARGV,  $remcommand ;
