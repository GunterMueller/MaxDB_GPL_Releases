 ...
 *
 * ASBeautifier.cpp   (11 February 2000)
...
/**
 * set the state of the modul frame option. 
 * If true, a given modul frame is skiped.
 *
 * @param   state             state of option.
 */
void ASBeautifier::setSkipModulFrame(bool state)
{
    skipModulFrame = state;
}
...
string ASBeautifier::beautify(const string &originalLine)
{
    ...
    // handle concom commands - inserted by h.b. 2001/05/11
    if (isCStyle && !isInComment && (originalLine[0] == '&'))
    {
            string concom = originalLine.substr(1, originalLine.find_first_of (' ')-1);


            if ((concom.COMPARE(0, 6, string("define")) == 0) ||
                (concom.COMPARE(0, 5, string("undef"))  == 0) ||
                (concom.COMPARE(0, 5, string("ifdef"))  == 0) ||
                (concom.COMPARE(0, 6, string("ifndef")) == 0) ||
                (concom.COMPARE(0, 2, string("if"))     == 0) ||
                (concom.COMPARE(0, 4, string("elif"))   == 0) ||
                (concom.COMPARE(0, 4, string("else"))   == 0) ||
                (concom.COMPARE(0, 5, string("endif"))  == 0) ||
                (concom.COMPARE(0, 5, string("ifvar"))  == 0) ||
                (concom.COMPARE(0, 6, string("ifnvar")) == 0) )
				return originalLine;
    ...
    {
        return activeBeautifierStack->back()->beautify(originalLine); // h.b changed line to originalLine - 2001/05/18
    }
    ...
    if (isCStyle && isInClass && classIndent && headerStackSize >= 2
            ...
            && (*headerStack)[headerStackSize-1] == &AS_OPEN_BRACKET && line[0] == '}')
        --tabCount;
    ...
    for (i=0; i<line.length(); i++)
    {
        ...
        if (ch == ':')
        {
            ...
            {
                if (prevNonSpaceCh == ')') // inserted by h.b. 2001/05/17
				{
                    // found a 'method (int parameter) : membervar (parameter)'
