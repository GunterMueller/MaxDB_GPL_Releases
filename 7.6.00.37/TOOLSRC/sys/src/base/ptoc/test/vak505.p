...
VAR
      ...
      PROCEDURE
        ...
        PROCEDURE
              a06put_errtext (
                    VAR acv       : tak_all_command_glob;
                    b_err_in      : tgg_basis_error;
                    err_type      : tgg_errortext;
                    errtext_len   : integer;
                    VAR errtext   : tsp_c256;
                    VAR b_err_out : tgg_basis_error);
        EXTERNAL ;
 ...
 (* PTS 1115085 *)
      ...
      PROCEDURE
              a507last_command_part (
                    ...
                    VAR parsk      : tak_parskey;
                    VAR resnam     : tsp_knl_identifier;
                    VAR w_link     : boolean;
                    qbufp          : tak_sysbufferaddress;
                    VAR ssr_rec    : tgg07_selectstrat_rec;
                    pos            : integer;
                    mtype          : tgg_message_type;
                    m2type         : tgg_message2_type;
      ...
      PROCEDURE
        ...
        PROCEDURE
              g04mblock_optimize_info (VAR mblock : tgg_mess_block);
        EXTERNAL ;
      ...
      PROCEDURE
        ...
        PROCEDURE
              g10mv3  (
                    ...
                    size1          : tsp_int4;
                    size2          : tsp_int4;
                    VAR val1       : tak_column_change_arr;
                    p1             : tsp_int4;
                    VAR val2       : tak_column_change_arr;
                    p2             : tsp_int4;
                    cnt            : tsp_int4;
                    VAR e          : tgg_basis_error);
        EXTERNAL ;
        ...
        PROCEDURE
              g10mv7  (
                    ...
                    VAR val1       : tsp_buf;
                    p1             : tsp_int4;
                    VAR val2       : tak_selfref_link_info;
                    p2             : tsp_int4;
                    cnt            : tsp_int4;
                    VAR e          : tgg_basis_error);
        EXTERNAL ;
...
PROCEDURE
      a505check_if_executed (
            VAR acv        : tak_all_command_glob;
            VAR test_name  : tsp_knl_identifier;
...
PROCEDURE
      a505get_longlit_rec (
            VAR acv            : tak_all_command_glob;
            VAR sparr          : tak_syspointerarr;
            VAR longlit_rec    : tak_sysbufferaddress);
        FORWARD;
...
PROCEDURE
      a505strategy_search (
            ...
            VAR dmli         : tak_dml_info;
            mtype            : tgg_message_type;
            VAR searchname   : tsp_knl_identifier;
...
PROCEDURE
      ak505make_st_entry (
            VAR mblock   : tgg_mess_block;
            index        : tsp_int2;
            e_type       : tgg_stack_entry_type;
            e_op         : tgg_stack_op_type;
            pos          : tsp_int2;
            len          : tsp_int2;
            tab1         : char;
            tab2         : char);
...
s10mv1 (sizeof(parsk), sizeof(syskey.sauthid), parsk, 1,
...
WITH acv, dmli, frec DO
    ...
    THEN
        ...
        aux_mb_data_len := pmbp^.mb_data_len;
        IF  (NOT full_part2) AND (pmbp^.mb_type <> m_fetch) AND
            (pmbp^.mb_data_len > 0)       AND
            (pmbp^.mb_data <> NIL)
        THEN
            pmbp^.mb_data_len := 0;
        (*ENDIF*)
        a06cpy_mblock (acv, pmbp^, a_mblock, b_err);
        pmbp^.mb_data_len    := aux_mb_data_len;
                ...
                ELSE
                    ...
                    THEN
                        ...
                        ELSE
                            ...
                            IF  ((a_mblock.mb_type = m_select_row) OR
                                p_single)
                            THEN
                                mtree.fileHandling_gg00 := mtree.fileHandling_gg00 + [ hsConsistentLock_egg00 ] -
                                      [ hsWithoutLock_egg00 ]
                            ELSE
                                IF  a_mblock.mb_type = m_select
                                THEN
                                    mtree.fileHandling_gg00 := mtree.fileHandling_gg00 + [ hsCollisionTest_egg00 ] -
                                          [ hsWithoutLock_egg00 ];
                                (*ENDIF*)
...
THEN
    WITH sparr.px[ 1 ]^.sparsinfo DO
        ...
        IF  p_changeinfos_exist AND p_select AND p_single
        THEN
            BEGIN
...
THEN
    WITH sparr.px[ 1 ]^.sparsinfo DO
        ...
        THEN
            BEGIN
            sysk := sparr.px[ 1 ]^.syskey;
            sysk.sentrytyp := cak_elongliteral;
            a10get_sysinfo (acv, sysk, d_fix, longlit_rec, b_err);
            IF  b_err <> e_ok
            THEN
                a07_b_put_error (acv, b_err, 2)
            ELSE
                BEGIN
                longlit_rec^.slongliteral.litcolcount := 0;
                t01buf (ak_sem, longlit_rec^, 1, longlit_rec^.b_sl);
                END;
            (*ENDIF*)
            END
        ELSE
...
WITH acv, dmli, intern_erec DO
    ...
    IF  (a_command_kind <> single_command) AND
        ...
        (NOT inner_loop)
    THEN
        old_res_tree := a_ptr2^.scomplexrec.comptree
    ELSE
    ...
    WHILE ((a_return_segm^.sp1r_returncode = 0) AND
        ...
        THEN
            ...
            THEN
                WITH jvrec, cinfo DO
                    ...
                    jv_checkview    := d_sparr.px[ 1 ]^.sparsinfo.p_reuse;
                    ex_with_link    := false;
                    check_after_dml :=
                    ...
                    IF  ((a_return_segm^.sp1r_returncode = a07_return_code(e_row_not_found,
                        a_sqlmode))
                        AND check_after_dml)
                    THEN
                        BEGIN
        ...
        THEN
            ...
            IF  ((old_res_tree.fileTfnTemp_gg00 = ttfnInternResult_egg00) OR
                ...
                (a_return_segm^.sp1r_returncode <> 100) AND
        ...
        IF  (a_command_kind <> single_command) AND
            (a_union_cnt    <  2)
        THEN
            a_ptr2^.scomplexrec.comptree := ex_act_treeid;
        (*ENDIF*)
        ...
        THEN
            ...
            a06subquery_del_result (acv,
                  dmli, sub_del_help_tree, mxgg_tempprefix + 3);
    ...
    (*ENDIF*)
    t01int4 (ak_sem , 'd_sublevel  ', dmli.d_sublevel);
    t01int4 (ak_sem , 'returncode  ', a_return_segm^.sp1r_returncode);
    IF  a_return_segm^.sp1r_returncode <> 0
    THEN
        BEGIN
        ...
        THEN
            a06subquery_del_result (acv, dmli,
                  sub_del_help_tree, mxgg_tempprefix+2);
        (*ENDIF*)
...
VAR
      ...
      _i_length             : integer;
      _pos                  : integer;
      _h_rescount           : tsp_int4;
      _f_ok                 : boolean;
      _re                   : tsp_num_error;
      _munion_cnt           : tsp_int4;
      _gg_strategy_ptr      : ^tgg07_StrategyInfo;
      _mtype                : tgg_message_type;
      _m2type               : tgg_message2_type;
      _resnam               : tsp_knl_identifier;
      _searchname           : tsp_knl_identifier;
      _join_parsk           : tak_parskey;
      _aux_treeid           : tgg00_FileId;
      _qbufp                : tak_sysbufferaddress;
      _ssr_rec              : tgg07_selectstrat_rec;
      _aux_tfn              : tgg00_TfnTemp;
      _union_kind           : tsp00_Uint1;
      _pmbp                 : tgg_mess_block_ptr;
      _pdbp                 : tgg_datapart_ptr;
...
THEN
    ...
    THEN
        ...
        THEN
            BEGIN
            IF  ( dmli.d_mselect_loop_cnt = cak_is_undefined )
            THEN
                a663_resname( acv, _resnam,
                      dmli.d_sparr.px[ 1 ]^.sparsinfo.p_modul_name,
...
THEN
    ...
    ELSE
            ...
            WITH cinfo DO
                ...
                ELSE
                    ...
                    g10mv13 ('VAK505',   6,
                          ...
                          erec.ex_act_treeid, 1, mxgg_treeid,
                          acv.a_return_segm^.sp1r_returncode);
                    END;
                (*ENDIF*)
                END
                ...
                CASE  acv.a_mblock.mb_type OF
                    ...
                    OTHERWISE
                        ...
                        THEN
                            ...
                            THEN
                                a506fieldvalues (acv, dmli, _frec,
                                      viewkeybuf, erec.ex_updjv_ptr^, sizeof(tsp_buf) (* see vak501 *))
                            ELSE
                                a506fieldvalues (acv, dmli, _frec,
                                      viewkeybuf,
                                      acv.a_mblock.mb_data^.mbp_buf, acv.a_mblock.mb_data_size)
        ...
        THEN
            ...
            THEN
                BEGIN
                IF  dmli.d_sparr.px[1]^.sparsinfo.p_const_param_expr
                THEN
                    a505const_param_expression ( acv, dmli,
                          acv.a_mblock );
                (*ENDIF*)
                t01messblock (ak_sem, 'MBLOCK 505mo', acv.a_mblock);
                acv.a_return_segm^.sp1r_returncode := cak_e_dbproc_call (* PTS 1112612 *)
                END;
            (*ENDIF*)
            END;
        (*ENDIF*)
        ...
        THEN
            ...
            ELSE
                IF  (_m2type = mm_empty) AND (_mtype = m_delete)
                THEN
                    _m2type := mm_qual;
                (*ENDIF*)
            (*ENDIF*)
            ...
            ELSE
                ...
                ELSE
                    ...
                    IF  ((_mtype  in [ m_select, m_show ]) AND
                        ...
                        OR
                        ((_m2type in [ mm_qual, mm_qual_no_opt, mm_empty ]) AND
                    ...
                    THEN
                        ...
                        THEN
                            BEGIN
                            t01int4 (ak_sem, 'loop_cnt    ',
                                  dmli.d_mselect_loop_cnt);
                            IF  (dmli.d_mselect_loop_cnt = 0)
                                OR
                                (((dmli.d_mselect_loop_cnt = -2) OR
                                (  dmli.d_mselect_loop_cnt = -3)) AND
            ...
            THEN
                WITH dmli.d_sparr.px[ 1 ]^.sparsinfo DO
                    ...
                    t01int4    (ak_sem, 'last_single ',
                          ord (_last_single_no_union));
                    ...
                    THEN
                        ...
                        acv.a_resname_addr[ cak_extern_pos ]^.sresname.restreeid.
                              fileTempCnt_gg00 := acv.a_max_res_id;
                        END;
                    (*ENDIF*)
                    END;
                (*ENDWITH*)
                ...
                THEN
                    ...
                    ELSE
                        ...
                        IF  ((_b_err = e_no_next_record)  OR
                            ...
                            mxgg_mb_part1head + mxgg_mb_part1_return) AND
                            ...
                            THEN
                                ...
                                THEN
                                    IF  NOT ((_mtype = m_select) AND
                                        (dmli.d_mselect_loop_cnt <> 1) AND
                                        (dmli.d_mselect_loop_cnt <> 0) AND
                                        (dmli.d_mselect_loop_cnt <>-3) AND
                                        (dmli.d_mselect_loop_cnt <>-1))
                                    THEN
                                    ...
                                    ELSE
                                        ...
                                        THEN
                                            ...
                                            IF  (acv.a_return_segm^.sp1r_returncode = 0) AND
                                                ((dmli.d_mselect_loop_cnt =  1) OR
                                                ( dmli.d_mselect_loop_cnt =  0) OR
                                                ( dmli.d_mselect_loop_cnt = -3) OR
                                                ( dmli.d_mselect_loop_cnt = -1))
                                            THEN
...
WITH acv DO
    ...
    WITH ke DO
        ...
        s10mv1 (sizeof(old_parsk), sizeof(sauthid), old_parsk, 1,
        ...
        THEN
            IF  b_err = e_sysinfo_not_found
            THEN
                a07_b_put_error (acv, e_old_fileversion,
...
WITH acv.a_mblock, mb_qual^, mstack_desc DO
    ...
    THEN
        ...
        (* PTS 1108845 E.Z. *)
        (*ENDIF*)
        IF  ( _gg_strategy_ptr^.str_strategy in
            [ strat_key_equal, strat_key_equal_sub ] )
            AND
        ...
        THEN
            ...
            IF  ( _StratInfo_len > sizeof( _gg_strategy ))
            THEN
                _StratInfo_len := sizeof( _gg_strategy );
            (*ENDIF*)
            IF  ( acv.a_return_segm^.sp1r_returncode = 0 )
            THEN
                BEGIN
                g10mv6  ('VAK505',  10,
                      ...
                      _StratInfo_len,
                      acv.a_return_segm^.sp1r_returncode);
                mb_strat_len := mb_st^[ mstrat_pos ].epos + _StratInfo_len - 1;
                mb_st^[ mstrat_pos ].elen_var := _StratInfo_len;
        ...
        THEN
            ...
            THEN
                ...
                ELSE
                    ...
                    IF  ( _b_err = e_ok )
                    THEN
                        WITH qbufp^.smessblock.mbr_mess_block DO
                            mb_st^[ mb_qual^.mstrat_pos ].etype :=
                                  st_dummy
                        (*ENDWITH*)
                    ELSE
                        a07_b_put_error (acv, _b_err, 1)
...
WITH  mblock, mb_qual^ DO
    BEGIN
    cal_begin  := index + 1;
    cal_end    := index + ord(mb_st^[ index ].ecol_tab [ 2 ]) + 1;
    t01int4 (ak_sem, 'cal_begin   ',cal_begin);
    t01int4 (ak_sem, 'cal_end     ',cal_end);
    restore_st := mb_st^[ cal_end ];
    mqual_pos  := cal_begin;
    mqual_cnt  := cal_end - cal_begin + 1;
    WITH mb_st^[ cal_end ] DO
        BEGIN
        etype         := st_output;
        eop_out       := op_o_output_not_fill;
        epos          := 1;
        elen_var      := cak_maxfieldlength+1;
        ecol_tab[ 1 ] := chr(0);
        ecol_tab[ 2 ] := chr(1);
        END;
    (*ENDWITH*)
    ...
    a05qualification_test (acv, mblock,
          c_result_wanted, c_check_new_rec,
          acv.a_cmd_part^.sp1p_buf_len,
          mb_data^.mbp_rec, result_buf, sizeof(result_buf), len);
    IF  acv.a_return_segm^.sp1r_returncode <> 0
    THEN
    ...
    part2_free := 0;
    i := succ(index);
    param_pos := mb_st^[ index ].epos;
    WHILE ( i < cal_end) AND
          ( mb_st^[ i ].etype = st_value) AND
          ( mb_st^[ i ].epos <> param_pos) DO
        i := succ(i);
    (*ENDWHILE*)
    WHILE i < cal_end  DO
        BEGIN
        IF  mb_st^[ i ].etype = st_value
        THEN
            part2_free := part2_free + mb_st^[ i ].elen_var;
        (*ENDIF*)
        i := succ(i);
        END;
    (*ENDWHILE*)
    t01int4 (ak_sem,'result len  ',len);
    t01int4 (ak_sem,'free part2  ', part2_free);
    mb_st^[ cal_end ] := restore_st;
    enough_place      := len <= part2_free
    END;
(*ENDWITH*)
...
WITH mblock, mb_qual^ DO
    BEGIN
    cal_begin  := index + 1;
    cal_end    := index + expr_cnt + 1;
    g10mv12 ('VAK505',  11,
          mb_st_size, mb_st_size,
          mb_st^, mxgg_stack_entry * index + 1,
          mb_st^, mxgg_stack_entry * (index - 1) + 1,
          mxgg_stack_entry * ( cal_end - cal_begin),
          acv.a_return_segm^.sp1r_returncode);
    WITH mb_st^[ cal_end - 1] DO
        BEGIN
        etype    := st_op;
        eop      := operator;
        epos     := 0;
...
VAR
      len          : integer;
      expr_cnt     : tsp_int2;
      enough_place : boolean;
      result_buf   : tsp_buf;
      operator     : tgg_stack_op_type;

BEGIN
cccprint('AK505 ak505compa');
WITH mblock, mb_qual^, dmli DO
    BEGIN
    operator := mb_st^[ index ].eop;
    expr_cnt := ord(mb_st^[ index ].ecol_tab[ 2 ]);
    ak505calculate_param_expression ( acv, mblock,
          result_buf, len, index, enough_place);
    IF  enough_place
    THEN
        BEGIN
        g10mv14 ('VAK505',  12,
              sizeof(result_buf), mb_data_size,
              result_buf, 1,
              mb_data^.mbp_buf, mb_st^[ index ].epos,
              len, acv.a_return_segm^.sp1r_returncode);
        ak505make_st_entry (mblock, index, st_value,
              operator, mb_st^[ index ].epos, len, chr(0), chr(0));
        ak505st_code_jump (mblock, index + 1, expr_cnt - 1);
        END
    ELSE
        BEGIN
        ak505st_code_rollback_for_const_expr (acv, mblock,
              index, expr_cnt, operator);
        d_only_table_scan := true;
        END;
    (*ENDIF*)
    index := index + ord(mb_st^[ index ].ecol_tab[ 2 ]) - 1;
    END;
(*ENDWITH*)
...
VAR
      ...
      i,j          : integer;
      len          : integer;
      expr_cnt     : ARRAY [ 1..2 ] OF tsp_int2;
      st_end       : tsp_int2;
      end_jump     : tsp_int2;
      roll_cnt     : tsp_int2;
      result_buf   : tsp_buf;
      operator     : tgg_stack_op_type;
      store_entry  : tgg_stack_entry;
      expression   : ARRAY [ 1..2 ] OF boolean;
      st_rollback  : ARRAY [ 1..2 ] OF boolean;
      st_index     : ARRAY [ 1..2 ] OF tsp_int2;

BEGIN
cccprint('AK505 ak505betwe');
WITH mblock, mb_qual^, dmli DO
    BEGIN
    st_end := index + ord(mb_st^[ index + 1 ].ecol_tab[ 2 ]) + 1;
    st_rollback [ 1 ] := false;
    st_rollback [ 2 ] := false;
    expression [ 1 ]  :=
          (mb_st^[ index ].ecol_tab[ 1 ] = cgg_param_in_between_expr);
    expression [ 2 ]  :=
          (mb_st^[ index + 1 ].ecol_tab[ 1 ] = cgg_param_in_between_expr);
    end_jump          := ord( mb_st^[ index + 1 ].ecol_tab[ 2 ]);
    IF  expression [ 1 ]
    THEN
        BEGIN
        st_index [ 1 ] := index + 2;
        st_index [ 2 ] := index + 3 + ord(mb_st^[ index + 2 ].ecol_tab[ 2 ]);
        END
    ELSE
        BEGIN
        st_index [ 1 ] := 0;
        ...
        END;
    (*ENDIF*)
    expr_cnt[ 1 ] := ord( mb_st^[ st_index [ 1 ] ].ecol_tab[ 2 ]);
    expr_cnt[ 2 ] := ord( mb_st^[ st_index [ 2 ] ].ecol_tab[ 2 ]);
    t01int4(ak_sem,'st_index[1] ',st_index[1]);
    t01int4(ak_sem,'st_index[2] ',st_index[2]);
    t01int4(ak_sem,'index       ',index);
    operator := op_none;
    FOR i := 1 TO 2 DO
        BEGIN
        ...
        THEN
            ...
            THEN
                BEGIN
                ak505calculate_param_expression ( acv, mblock, result_buf,
                      len, st_index [ i ], enough_place);
                IF  enough_place
                THEN
                    g10mv14 ('VAK505',  13,
                          sizeof(result_buf), mb_data_size,
                          result_buf, 1,
                          mb_data^.mbp_buf, mb_st^[ st_index [ i ] ].epos,
                          len, acv.a_return_segm^.sp1r_returncode)
                ELSE
                    st_rollback[ i ] := true;
                (*ENDIF*)
                END
            ELSE
                len := mb_st^[st_index [ i ] ].elen_var;
            (*ENDIF*)
            IF  NOT st_rollback[ i ]
            THEN
                BEGIN
                ak505make_st_entry (mblock, index + i - 1, st_value,
                      operator, mb_st^[st_index [ i ] ].epos, len,
                      chr(0) ,chr(0));
                ak505st_code_jump (mblock, st_index [ i ],
                      expr_cnt [ i ]);
                END;
            (*ENDIF*)
            END;
        (*ENDIF*)
        operator := mb_st^[ index + 1 ].eop;
        END;
    (*ENDFOR*)
    IF  NOT st_rollback [ 1 ] AND NOT st_rollback [ 2 ]
    THEN
        index := index + 2;
    (*ENDIF*)
    roll_cnt    := 0;
    store_entry := mb_st^[ index + 1 ];
    IF  st_rollback [ 1 ]
    THEN
        BEGIN
        roll_cnt := succ(roll_cnt);
        g10mv12 ('VAK505',  14,
              mb_st_size, mb_st_size,
              mb_st^, mxgg_stack_entry * (st_index [ 1 ] ) + 1,
              mb_st^, mxgg_stack_entry * (index - 1) + 1,
              mxgg_stack_entry *  expr_cnt[ 1 ],
              acv.a_return_segm^.sp1r_returncode);
        index := index + expr_cnt[ 1 ];
        end_jump := end_jump - expr_cnt[ 1 ] + 1;
        mb_st^[ index ] := store_entry;
        mb_st^[ index ].ecol_tab[ 2 ] := chr(end_jump);
        IF  NOT st_rollback [ 2 ]
        THEN
            ak505st_code_jump (mblock, index + 1, expr_cnt[ 2 ] + 2 );
        (*ENDIF*)
        END
    ELSE
        index := succ(index);
    ...
    THEN
        ...
        t01int4 (ak_sem, 'st_index[2] ',st_index[ 2 ]);
        t01int4 (ak_sem, 'index       ',index);
        t01int4 (ak_sem, 'expr_cnt[2] ',expr_cnt[ 2 ]);
        FOR j := 1 TO mfirst_free DO
            t01stackentry(ak_sem, mb_st^[ j ], j);
        (*ENDFOR*)
        g10mv12 ('VAK505',  15,
              mb_st_size, mb_st_size,
              mb_st^, mxgg_stack_entry * (st_index [ 2 ] ) + 1,
              mb_st^, mxgg_stack_entry * (index - 1) + 1,
              mxgg_stack_entry *  expr_cnt[ 2 ],
              acv.a_return_segm^.sp1r_returncode);
        index := index + expr_cnt[ 2 ];
        end_jump := end_jump - expr_cnt[ 2 ] + 1;
        ak505make_st_entry (mblock, index, st_op, store_entry.eop,
              0, 0, chr(0), chr(end_jump));
        IF  NOT st_rollback [ 1 ]
        THEN
            ak505st_code_jump (mblock, index + 1, expr_cnt[ 1 ] + 1)
        ELSE
        ...
        END;
    (*ENDIF*)
    d_only_table_scan := st_rollback[ 1 ] OR st_rollback [ 2 ];
    END;
(*ENDWITH*)
index := st_end;
...
VAR
      enough_place   : boolean;
      st_rollback    : boolean;
      real_entry_ptr : tsp_int2;
      st_move_offset : tsp_int2;
      st_begin       : tsp_int2;
      st_end         : tsp_int2;
      expr_cnt       : tsp_int2;
      len            : integer;
      result_buf     : tsp_buf;
      stack_list     : tgg_stack_list_ptr;
...
WITH mblock, mb_qual^, dmli DO
    ...
    st_begin    := index;
    st_rollback := false;
    WHILE (mb_st^[ index ].etype <> st_op) AND
          (mb_st^[ index ].eop   <> op_in) DO
        BEGIN
        IF  (mb_st^[ index ].etype = st_value) AND
            (mb_st^[ index ].ecol_tab[ 1 ] = cgg_param_in_in_expr)
        THEN
            BEGIN
            real_entry_ptr := index + ord(mb_st^[ index ].ecol_tab[ 2 ]);
            expr_cnt       := ord(mb_st^[ real_entry_ptr ].ecol_tab[ 2 ]);
            ak505calculate_param_expression ( acv, mblock, result_buf,
                  len, real_entry_ptr, enough_place);
            IF  enough_place
            THEN
                BEGIN
                g10mv14 ('VAK505',  16,
                      sizeof(result_buf), mb_data_size,
                      result_buf, 1,
                      mb_data^.mbp_buf, mb_st^[ real_entry_ptr ].epos,
                      len, acv.a_return_segm^.sp1r_returncode);
                ak505make_st_entry (mblock, index, st_value,
                      op_none, mb_st^[real_entry_ptr ].epos, len,
                      chr(0) ,chr(0));
                ak505st_code_jump (mblock, real_entry_ptr,
                      expr_cnt);
                END
            ELSE
                BEGIN
                st_rollback := true;
                mb_st^[ real_entry_ptr ].ecol_tab[ 2 ] := chr (expr_cnt);
                END;
            (*ENDIF*)
            END;
        (*ENDIF*)
        ...
        END;
    (*ENDWHILE*)
    st_end := index + ord( mb_st^[ index ].ecol_tab[ 2 ]) - 1;
    IF  st_rollback
    THEN
        BEGIN
        index  := st_begin;
        ...
        WHILE (mb_st^[ index ].etype <> st_op) AND
            ...
            THEN
                ...
                real_entry_ptr := index + ord(mb_st^[ index ].ecol_tab[ 2 ]) -
                      st_move_offset;
                expr_cnt       := ord(mb_st^[ real_entry_ptr ].ecol_tab[ 2 ]);
                st_move_offset := st_move_offset + expr_cnt - 1;
                t01int4 ( ak_sem, 'st_rollback ',4711);
                t01int4 ( ak_sem, 'real_entry_p',real_entry_ptr);
                t01int4 ( ak_sem, 'expr_cnt    ',expr_cnt);
                t01int4 ( ak_sem, 'index       ',index);
                a10new (acv, expr_cnt * mxgg_stack_entry, stack_list);
                IF  stack_list = NIL
                THEN
                    a07_b_put_error (acv, e_no_more_memory, 1)
                ELSE
                    BEGIN
                    g10mv12 ('VAK505',  17,
                          mb_st_size, expr_cnt*mxgg_stack_entry,
                          mb_st^, real_entry_ptr * mxgg_stack_entry + 1,
                          stack_list^, 1, expr_cnt * mxgg_stack_entry,
                          acv.a_return_segm^.sp1r_returncode);
                    g10mvr ('VAK505',  18,
                          mb_st_size, mb_st_size,
                          mb_st^, (index ) * mxgg_stack_entry + 1,
                          mb_st^, (index  + expr_cnt - 1 ) * mxgg_stack_entry + 1,
                          (real_entry_ptr  - index - 1) * mxgg_stack_entry,
                          acv.a_return_segm^.sp1r_returncode);
                    g10mv12 ('VAK505',  19,
                          expr_cnt*mxgg_stack_entry, mb_st_size,
                          stack_list^, 1,
                          mb_st^, (index - 1) * mxgg_stack_entry + 1,
                          expr_cnt * mxgg_stack_entry,
                          acv.a_return_segm^.sp1r_returncode);
                    ak505st_code_jump (mblock,
                          real_entry_ptr + expr_cnt - 1, 1);
            ...
            END;
        (*ENDWHILE*)
        mb_st^[ index ].ecol_tab[ 2 ] :=
              chr(ord(mb_st^[ index ].ecol_tab[ 2 ]) -
              st_move_offset - 1);
        END;
    (*ENDIF*)
    index := st_end;
    d_only_table_scan := st_rollback;
    END;
(*ENDWITH*)
...
VAR
      aux_mqual_cnt       : tsp_int2;
      aux_mqual_pos       : tsp_int2;
      m_cnt               : tsp_int2;
      m_pos               : tsp_int2;
      index               : tsp_int2;
      opt_ptr             : tsp_moveobj_ptr;
      m_opt_pos           : integer;
...
WITH  mblock, mb_qual^ DO
    BEGIN
    aux_mqual_cnt := mqual_cnt;
    aux_mqual_pos := mqual_pos;
    m_cnt         := mqual_cnt;
    m_pos         := mqual_pos;
    IF  mst_optimize_pos > 0
    THEN
        opt_ptr := @mb_data^.mbp_buf[mst_optimize_pos];
    (*ENDIF*)
    REPEAT
        index := m_pos;
        WHILE (index < m_cnt + m_pos) DO
            BEGIN
            m_opt_pos := 0;   (* h.b. PTS 1001399 *)
            IF  (mb_st^[ index ].etype = st_value) AND
            ...
            THEN
                BEGIN
                IF  mst_optimize_pos > 0 (* h.b. PTS 1001399 *)
                THEN (* invalidate optimize info *)
                    BEGIN
                    (* h.b. PTS 1001399 *)
                    m_opt_pos        := mst_optimize_pos;
                    mst_optimize_pos := 0;
                    (* PTS 1106590 E.Z. *)
                    IF  ord(mb_st^[ index ].eop) = ord(opt_ptr^[index - aux_mqual_pos + 1])
                    THEN
                        opt_ptr^[index - aux_mqual_pos] := chr(0);
                    (*ENDIF*)
                    opt_ptr^[index - aux_mqual_pos + 1] := chr(0);
                    END;
                (*ENDIF*)
                CASE mb_st^[ index ].ecol_tab [ 1 ] OF
                    chr(0) :
                        ak505compare_output_expr ( acv, dmli, mblock, index);
                    cgg_param_in_between_expr :
                        BEGIN
                        IF  mb_st^[ index ].eop in
                        ...
                        END;
                    cgg_param_in_in_expr :
                        ak505in_expr ( acv, dmli, mblock, index);
                    OTHERWISE
                        mb_trns^.trError_gg00 := e_stack_type_illegal;
                    END;
                (*ENDCASE*)
                END;
            (*ENDIF*)
            IF  m_opt_pos > 0 (* h.b. PTS 1001399 *)
            THEN
                BEGIN
                mst_optimize_pos := m_opt_pos;
                m_opt_pos        := 0;
                END;
            (*ENDIF*)
            index := succ(index);
        ...
        THEN
            ...
            WHILE index < m_cnt + m_pos DO
                BEGIN
                IF  (mb_st^[ index ].etype = st_jump_absolute) AND
                    (mb_st^[ index + mb_st^[ index ].epos + 1 ].etype
                    = st_jump_absolute)
                THEN
                    BEGIN
                    WITH mb_st^[ index + mb_st^[ index ].epos + 1 ] DO
                        BEGIN
                        etype := st_not_needed;
                        eop   := op_none;
                        END;
                    (*ENDWITH*)
                    mb_st^[ index ].epos := mb_st^[ index ].epos +
                          mb_st^[ index + mb_st^[ index ].epos + 1 ].epos + 1;
                    index := pred(index);
                    END;
                (*ENDIF*)
                index := succ(index);
    ...
    UNTIL
        (index > mresqual_pos);
    (*ENDREPEAT*)
    mqual_cnt := aux_mqual_cnt;
    mqual_pos := aux_mqual_pos;
    END;
(*ENDWITH*)
...
VAR
      ...
      lo_level      : tsp_int2;
      use_file_id   : tsp_int2;
      self_ref_info : tak_selfref_link_info;

BEGIN
cccprint('AK505 ak505exec_');
use_file_id := 0;
WHILE (pars_id <> 0) AND (acv.a_return_segm^.sp1r_returncode = 0) DO
    BEGIN
...
VAR
      ...
      out_cnt       : integer;
      desc_len      : integer;
      long_desc_pos : tsp_int4;
      offset        : tsp_int4;
      long_desc     : tsp_long_descriptor;
      mblock        : tgg_mess_block;
      curr_qual_buf : tgg_qual_buf;
      curr_data_buf : tgg_rec_buf;
...
WITH acv, dmli, long_desc DO
    ...
    (*ENDIF*)
    IF  (mtype = m_insert) AND (a_return_segm^.sp1r_returncode = 0)
    THEN
        BEGIN
        long_desc_pos := d_first_longdesc_pos;
        FOR cnt := 0 TO d_cnt_longdesc - 1 DO
            IF  a_return_segm^.sp1r_returncode = 0
            THEN
                BEGIN
                g10mv10 ('VAK505',  21,
                ...
                IF  (a_data_ptr^[long_desc_pos] <> csp_undef_byte) AND
                    NOT (ld_without_shortcol in long_desc.ld_infoset)
                THEN
                    BEGIN
                    ...
                    WITH mblock DO
                        ...
                        g01stack_init (mblock, NIL, 0,
                              @curr_qual_buf, sizeof (curr_qual_buf));
                        WITH mb_qual^ DO
                            BEGIN
                            mtree := dmli.d_sparr.px[ 1 ]^.
                                  sparsinfo.p_treeid;
                            IF  mtree.fileTfn_gg00 <> tfnTemp_egg00
                            THEN
                                mtree.fileHandling_gg00 := acv.a_transinf.
                                      tri_global_state
                            (*ENDIF*)
                            END;
                        (*ENDWITH*)
                        ...
                        mb_qual^.mtree.fileTfn_gg00:= tfnShortScol_egg00;
                        mb_qual^.mtree.fileVersion_gg00.ci2_gg00 := cgg_dummy_file_version;
                        mb_qual^.mtree.fileRoot_gg00 := NIL_PAGE_NO_GG00;
                        (* PTS 1113190 E.Z. *)
                        mb_data_len := cgg_rec_key_offset +
                              cgg_str_col_entry_size;
                        mb_data^.mbp_reclen := mb_data_len ;
                        mb_data^.mbp_keylen := cgg_str_col_entry_size;
                        mb_data^.mbp_varcol_offset := 0;
                        mb_data^.mbp_varcol_cnt    := 0;
                        g10mv9  ('VAK505',  22,
                              sizeof (long_desc.ld_descriptor),
                              mb_data_size, long_desc.ld_descriptor, 1,
                ...
                THEN
                    BEGIN
                    long_desc.ld_infoset := long_desc.ld_infoset +
                          [ ld_ex_trigger ];
                    g10mv11 ('VAK505',  23,
        ...
        END
    ELSE
        IF  ((mtype = m_update) AND (a_return_segm^.sp1r_returncode = 0))
        THEN
            FOR cnt := 0 TO d_cnt_longdesc - 1 DO
                BEGIN
                g10mv10 ('VAK505',  24,
                      ...
                      a_return_segm^.sp1r_returncode);
                IF  a_mblock.mb_qual_len  =
                    mxgg_mb_part1head+mxgg_mb_part1_return
                THEN
                    ld_descriptor := cgg_zero_id
                ELSE
                    BEGIN
                    g10mv1 ('VAK505',  25,
                          a_mblock.mb_qual_size, sizeof(ld_descriptor),
                          a_mblock.mb_qual^.buf,
                          mxgg_mb_part1head + mxgg_mb_part1_return +
                          cnt * (1 + sizeof(ld_descriptor)) + 2,
                          ld_descriptor, 1, sizeof(ld_descriptor),
                          a_return_segm^.sp1r_returncode);
                    IF  dmli.d_sparr.px[ 1 ]^.sparsinfo.p_mtyp2 = mm_expand
                    THEN
                        ld_intern_pos := cgg_eo_bytestr
                    ELSE
                        IF  (ld_without_shortcol in ld_infoset) AND
                            (a_mblock.mb_qual^.buf
                            [ mxgg_mb_part1head + mxgg_mb_part1_return +
                            cnt * (1 + sizeof(ld_descriptor)) + 1] = chr(true))
                        THEN
                            ld_intern_pos := -1
                        (*ENDIF*)
                    (*ENDIF*)
                ...
                IF  a_execute_trigger
                THEN
                    ld_infoset := long_desc.ld_infoset + [ ld_ex_trigger ];
                (*ENDIF*)
                g10mv11 ('VAK505',  26,
                      sizeof(long_desc), a_data_length,
                      long_desc, 1,
    ...
    WHILE ((a_return_segm^.sp1r_returncode = 0) AND
        ...
        t01moveobj (ak_sem, a_data_ptr^,
              d_first_longdesc_pos + cnt * (desc_len + 1),
              d_first_longdesc_pos + cnt * (desc_len + 1) + desc_len);
        ...
        THEN
            ...
            ELSE
                ...
                a507_putval (acv, d_first_longdesc_pos +
                      cnt * (desc_len + 1), c_intern_call);
                IF  (a_curr_retpart <> NIL) AND
                    (a_curr_retpart^.sp1p_buf_len > 0)
                THEN
                    BEGIN
...
(*
static char ident_mf[] = "@(#)vak505 SLOW I386 UNIX LINUX PTOC -REL30=1 -RELVER=R74 -DIAGNOSE=1 -BETA=1";
*)
