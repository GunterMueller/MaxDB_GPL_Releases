...
extern void erro2(char *s, void *a1);
extern void error(char *s);
extern void settab();
extern int compare(char *v);
extern FILE *copen(char *s, char c);

static void chstash(int c);
static void cpyact();
static void cpycode();
static void defout();
static void finact();
static int chfind(int t);
static int defin(int t);
static int gettok();
...
{	int i,j,lev,t;
	...
	while( argc >= 2  && argv[1][0] == '-' ) {
		while( *++(argv[1]) ){
			...
			case 'v':
			case 'V':
				foutput = copen((char *)"y.output", (char)'w' );
			...
			case 'R':
				...
				rflag = 1;
				continue;
			default:  erro2( "illegal option: %c", (void *)*argv[1]);
				}
			}
		argv++;
		argc--;
	...
	ftable = copen( oflag ? (char *)"yacc.tmp" : (char *)"y.tab.c" , (char)'w' );
	if( ftable==0 ) error( "cannot open table file" );
	if( argc > 1 ) { cin = copen( argv[1], (char)'r' );
	...
	while( ( t = gettok() ) != EOF ) {
		switch( t ){
			...
			case NUMBER:
				...
				if( j < ndefout && j>2 )
					erro2("please define type # of %s earlier", trmset[j].name );
				continue;
			default:	erro2("bad precedence syntax, input %d", (void *)t );
			}
		break;
		}
	prdptr[0]=mem;
	...
	*mem++ = 0;
	...
	case PREC:	
		...
		j=chfind(2);
		if(j>=NTBASE)erro2("nonterminal %s illegal after %%prec", nontrst[j-NTBASE].name);
	...
	case LCURLY:	
		...
		cpycode();
		continue;
	default: erro2( "syntax error, input %d", (void *)t );
	}
}
...
/*	define ctokn to be a terminal if t=0
	or a nonterminal if t=1		*/
        ...
        if (t) {
          if( ++nnonter >= ntlim ) erro2("too many nonterminals, limit %d",(void *)ntlim);
	  nontrst[nnonter].name = ctokn;
	  return( NTBASE + nnonter );
          }
        else {
          if( ++nterms >= tlim ) erro2("too many terminals, limit %d",(void *)tlim );
...
static void defout(){ /* write out the defines (at the end of the declaration section) */
...
begin:
	...
	if( reserve ){ /* find a reserved word */
		...
		if( compare("PREC")) return( PREC );
		erro2("invalid escape, or illegal reserved word: %s", ctokn );
		}
...
{	int i;
	...
	/* cannot find name */
	if( t>1 && ctokn[0] != ' ' )
		erro2( "%s should have been defined earlier", ctokn );
	return( defin( t ) );
	}

static void cpycode(){ /* copies code between \{ and \} */
...
static void cpyact(){ /* copy C action to the next ; or closing } */
