...
/* RASE err.6,: never assigned (temp.fix.)*/
#ifdef DEBUG
/*bool	fulltrace=1, errtrace=1, testtrace=1;*/
#endif /* DEBUG */
...
/*  EF4: 11-08-86 WB; additionally needed */
extern int val[], sp;
	...
	int *Ps0, idfail;
{
	...
	if (idfail) {
		 ...
		 */
		i = val[sp];				/* EF4: 11-08-86 WB */
		val[sp] = nullsem(id);			/* EF4: 11-08-86 WB */
...
#endif
		if (c < ccost)				/* EF4: 11-08-86 WB */
			cact = CCHIDENT, ccost = c, cchar = id;
	}

	/*
	 * First try correcting the state we are in
	 */
	trystate(Ps0, sp, 0, &insmult[1], &delmult[1], &repmult[1]);
							/* EF4: 11-08-86 WB */
	...
	if (OY.Yychar != -1 && OY.Yylval != nullsem(OY.Yychar)) {
		...
		trystate(Ps0 - 1, sp - 1, 1, insmult, delmult,
				repmult);		/* EF4: 11-08-86 WB */
		if (cflag == 0)
			YC++;
		else {
			sp--;				/* EF4: 11-08-86 WB */
 ...
 */
					/* EF4: 11-08-86 WB; parameter sp */
trystate(Ps0, psp, flag, insmult, delmult, repmult)
	int *Ps0, psp, flag;
	char *insmult, *delmult, *repmult;
{
...
#endif
							/* EF4: 11-08-86 WB */
		c = correct(NOCHAR, 1, c, delmult, Ps0, psp);
#ifdef DEBUG
		if (c < CPRLIMIT || fulltrace)
	...
	for (; *ap <= 0; ap += 2) {
		...
		if (c < ccost) {
			...
			if (c == 0) {
							/* EF4: 11-08-86 WB */
				c = correct(acchar, 0, 1, insmult + 1, Ps0, psp);
#ifdef DEBUG
				...
				if (c < ccost)
					cact = CUNIQUE, ccost = 0, cchar = acchar, cflag = flag;
							...
							/* EF4: 11-08-86 WB */
				c = correct(acchar, 0, c, insmult, Ps0, psp);
#ifdef DEBUG
				if (c < CPRLIMIT || fulltrace)
		...
		if (c < ccost) {
#endif
			c = correct(acchar, 1, repcost(YC[0].Yychar, acchar), repmult, Ps0, psp);
#ifdef DEBUG
			if (c < CPRLIMIT || fulltrace)
 ...
 */
					/* EF4: 11-08-86 WB; parameter sp */
correct(fchar, origin, c, multvec, Ps0, psp)
	...
	int *Ps0, psp;
{
	...
	yytips[0] = *ps;
	ps--;
	yytipv[0] = val[psp];				/* EF4: 11-08-86 WB */
	yCpv = psp - 1;					/* EF4: 11-08-86 WB */
	yytipct = 1;
