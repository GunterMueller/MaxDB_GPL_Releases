...
sub connect_qadb
{
	unless ($dbh)
	{
		...
		#$dbh = DBI->connect("DBI:MaxDB:BERD0080054596A/T_QADB","MSG_USER","MSG_USER") or die "Can't connect $DBI::err $DBI::errstr\n";
		dbgdbg("successfully\n");
	}
}
...
sub get_taglist_for_message
{
	my $component = shift;
	my $msgid = shift;
	my $releaseid = get_releaseid();
	my @tag_list = ();
	my $cmd = "SELECT TAG FROM V_MSG_REL_TAGS WHERE COMPONENT='$component' AND MESSAGEID=$msgid AND RELEASE=$releaseid ORDER BY TAG";
	connect_qadb();
	dbgdbg ("Look for existing tags for $component"."-$msgid ($releaseid) in tag table\n", 1);
	dbgdbg ("Execute: $cmd\n", 3);
	...
	unless ($ref)
	{
		...
		{  	dbgdbg("Info: no tag entries found\n", 1); }
	}
	else
	{
		@tag_list =  @$ref;
...
sub get_taghash_for_component
{
	...
	my $cmd = "SELECT MESSAGEID, TAG FROM V_MSG_REL_TAGS WHERE COMPONENT='$component' and RELEASE = $releaseid";
	my %tag_hash;
...
sub get_header_intro
{
	...
	push @new_intro_lines, split /\n/, "#error _file_ was probably integrated between releases, do not integrate this header file, \\\nuse buildmsg";
	push @new_intro_lines, ("#endif","","");		
...
sub create_msgid_area
{
	...
	my @new_msg_lines = ();
	my $tagcount = defined $single_message->{'TAGS'} ? scalar(@{$single_message->{'TAGS'}}) + 1 : 1;
...
sub new_messagefile
{
	...
	unless (defined $force_xml_creation)
	{
		...
		{
			if ("$file_last_date" ge "$db_last_date")
			{
...
sub create_xml_area
{
	...
	$single_message->{'DESCRIPTION'} and 
	      ( push @new_xml_lines, "<Description>", split /\n/, $single_message->{'DESCRIPTION'}, "</Description>" ); 
	$single_message->{'ACTIONNAME'} and
	      push @new_xml_lines, ("<Action>", split /\n/, get_action_text($single_message->{'ACTION'}), "</Action>");
...
sub revert
{
	...
	$sync_to_head and $rc = P4Lib::p4_sync($file);
	return 1;
}
...
sub checkout 
{
	...
	dbgdbg ("Check out of $file ". ( $no_sync ? "without" : "with" ) . " before\n",1 );
