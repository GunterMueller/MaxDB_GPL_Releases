...
/*!
  -----------------------------------------------------------------------------
  module: ParamGetProperties.java
  -----------------------------------------------------------------------------
 ...
 * <xmp>
 * -----------------------------------------------------------------------------
 * Class:        ParamGetProperties
 * -----------------------------------------------------------------------------
 * responsible:  ChristianH
 * special area: DBMServer Java Test
 * description:  DBMServer param_getproperties command tests
 * version:      7.4.*
 * Copyright (c) 2000-2005 SAP AG
 * -----------------------------------------------------------------------------
 * </xmp>
...
public class ParamGetProperties extends BackupState
{
  private Vector ParamLabel;

  public ParamGetProperties() throws SQLException, TestDatabaseException
  {
    super();

    ParamLabel = new Vector();
    //Expected label of param_getproperties MAXDATAVOLUMES
  ...
  public void run()
  {
    ...
    try
    {
      ...
      oDBM.execCmd("param_getproperties MAXDATAVOLUMES",DBMUtility.OK_OK);
      ...
      boolean m_paramok = false;
      //Checking the gotten label string
      for (int i = 0;i < ParamLabel.size(); i++) {
        if (ParamArray.get(i).toString().equals(ParamLabel.get(i).toString())) {
          m_paramok = true;
        } else {
          m_paramok = false;
        }
