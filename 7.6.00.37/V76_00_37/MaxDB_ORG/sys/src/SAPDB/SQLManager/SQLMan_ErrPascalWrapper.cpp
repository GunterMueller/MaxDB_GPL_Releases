/*!***************************************************************************
  ...
  module      : SQLMan_ErrPascalWrapper.cpp
...
void g171_baseerrortext (
    tgg00_BasisError                     b_err,
    char*                                text,
    integer                              textsize)
{
    const char *baseText = SQLManErr_Interface::GetInstance().GetBaseErrorText(b_err);
	if (NULL != baseText)
	{
        int msglen = strlen (baseText);
		if (msglen > textsize)
		{
			msglen = textsize;
		}
		SAPDB_MemCopyNoCheck (text, baseText, msglen);
    }
	return;
}
