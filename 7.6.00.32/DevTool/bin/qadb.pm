...
sub mark_testend {
	...
	if ($href->{'Rows'}->{'Row'}[0]->{'CRES'} >= 1) {
		...
		$self->check_lcok();
		$self->execDML("UPDATE makes SET idobjstatus = 3000 WHERE id = $self->{'id'}'");
	}
}
...
sub write_prot_to_webdav
{
	...
	if ($res->is_success()) {
	    ...
	    &throw_err("write_prot_to_webdav: Could not create file entry on the server: Message: $res->as_string\nFile name: $url\n".
	               "Content: " . (($ContentType =~ /zip/) ? "Zipfile\n" : "'$prot'\n"));
	    $rc = 1;
	}
	return $rc;
}
...
sub write_prot {
	...
	if (write_prot_to_webdav("http://$httpmachine:$httpport$current_path", $prot, $ContentType))
	{
		$self->{error_code} = 1;
        return 1;
	}
...
sub create_testprofile {
	...
	unless ($profile =~ /../) {
		$self->{'error_code'} = 1;
		$self->{'error_text'} = &throw_err("create_testprofile: No profile given!\n\$profile = $profile\n");
		return 1;
	}
	
	$profile = &quote_sql2($profile);
	
	$stmt = "SELECT id FROM testsets WHERE testset = $profile";
	($rc, $href) = QAConnect::httpsql_request("QADB_HTTPSQL", $stmt);
	unless ($rc) {
		$self->{error_code} = 1;
		$self->{error_text} = &throw_err("Troube while executing the following SQL-Statement:\n$stmt\n");
	}
		
	if ($href->{'Rows'}->{'Row'}[0]->{'ID'} > 1) {
		$idtestset = $href->{'Rows'}->{'Row'}[0]->{'ID'};
	} else {
		$stmt = "SELECT testsets_seq.NEXTVAL AS CRES FROM DBA.DUAL";
		($rc, $href) = QAConnect::httpsql_request("QADB_HTTPSQL", $stmt);
		unless ($rc) {
			$self->{error_code} = 1;
			$self->{error_text} = &throw_err("Troube while executing the following SQL-Statement:\n$stmt\n");
		...
		$stmt = "INSERT INTO testsets (id, testset) VALUES ( $idtestset, $profile )";
		($rc, $href) = QAConnect::httpsql_request("QADB_HTTPSQL", $stmt);
		unless ($rc) {
			$self->{error_code} = 1;
			$self->{error_text} = &throw_err("Troube while executing the following SQL-Statement:\n$stmt\n");
	...
	$stmt = "UPDATE makes SET idtestset = $idtestset WHERE id = " . $self->{'ID'};
	($rc, $href) = QAConnect::httpsql_request("QADB_HTTPSQL", $stmt);
	unless ($rc) {
		$self->{error_code} = 1;
		$self->{error_text} = &throw_err("Troube while executing the following SQL-Statement:\n$stmt\n");
	...
	$stmt = "SELECT * FROM makes WHERE id = $self->{'ID'}";
	($rc, $href) = QAConnect::httpsql_request("QADB_HTTPSQL", $stmt);
	unless ($rc) {
		$self->{error_code} = 1;
		$self->{error_text} = &throw_err("Troube while executing the following SQL-Statement:\n$stmt\n");
