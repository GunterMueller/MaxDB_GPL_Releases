...
class HTMLWriter:
    ...
    def openFile (self, outfile):
        ...
        stream.write ('<i>generated %s by getLcProtocols.py</i><br>\n' % timestamp)
        return stream
    ...
    def writeByVmake (self, report, stream):
        ...
        stream.write ('<tr><th>Platform</th><th>Release</th><th>Target</th><th>Version</th><th>Err/Warn</th><th>started</th></tr>\n')
        ...
        for platform, release, starttime, target, speed, run in runs:
            ...
            stream.write ('<tr><td>%s</td><td>%s</td><td><a href="%s">%s</a></td><td>%s</td><td>%s/%s</td><td>%s</td></tr>\n'
                % (platform, release, run.url, run.target, run.speed, errorString, run.warnings, run.starttime))
        stream.write ('</table>\n')
    ...
    def writeBySource (self, report, stream):
        ...
        lastEnv = (None, None, None)
        lastText = None
        stream.write ('<h3>Messages by Source</h3>\n')
        for source, release, platform, bitness, error, warning, entry in sorted:
            if source != lastSource:
                if lastSource:
                    stream.write ('</dl>\n')
...
_release2perforce = {
    '7403': '//sapdb/V74/develop',
    '7402': '//sapdb/V74/c_02/develop',
    '7403_DEV': '//sapdb/V74/develop',
