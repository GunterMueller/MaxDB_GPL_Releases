...
def parseTypes (tokens):
    #
    # check for ptoc00Redefine= comment
    #
    if tokens [-1].value == ';':
        lastToken = tokens [-2]
    else:
        lastToken = tokens [-1]
    redefine = lastToken.getSpecialComment ('postComment', 'ptoc00Redefine')
    if redefine:
        return RedefinedDcl (redefine, tokens)
    #
...
class RedefinedDcl:
    def __init__ (self, typestring, tokens):
    ...
    def __repr__ (self):
        return '<RedefinedDcl %s>' % self.typestring
