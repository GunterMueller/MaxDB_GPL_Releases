#
# dbapi.py An implementation of the Python database API 2.0 for MaxDB
#     contact: maxdb@lists.mysql.com.com
#
#
#    ========== licence begin  GPL
...
class Cursor:
    ...
    def __handleResult (self, sqlResult):
        result = None
        #
        # last serial
        #
        lastSerial = sqlResult.getLastSerial ()
        if lastSerial is not None:
            self.lastrowid = lastSerial
        #
        # rows affected
        #
        rowsAffected = sqlResult.getRowsAffected ()
        if rowsAffected >= 0:
            result = rowsAffected
        #
        # result set
        ...
        if resultSet is not None:
            self.__cursor = resultSet
            self.__setDescription (resultSet)
            if self.__typeTranslation:
                self.__instantiateTranslation ()
            if hasattr (self, 'fetchSize'):
                resultSet.setFetchSize (self.fetchSize)
            result = self
        else:
            self.__cursor = None
            self.description = None
        ...
        # output parameter
        #
        outparms = sqlResult.getOutputParameters ()
        if outparms is not None:
            self.__cursor = SingleRowResultSet (outparms)
            description = self.description = []
            for name, typename, guess, size, frac, maybeNull, inout in self.__parsed.getDescription ():
                if inout in ['IN', 'IN/OUT']:
                    description.append ((name, typename, size, size, size, frac, maybeNull))
            result = outparms
        return result
    ...
    def callproc (self, procname, parameters = None):
        ...
        for paramIndex in xrange (len (desc)):
            mode = desc [paramIndex] [-1]
            if mode == 'OUT':
                outmapping.append (paramIndex)
            elif mode == 'IN/OUT':
                outmapping.append (paramIndex)
                inparameters.append (parameters [paramIndex])
            elif mode == 'IN':
                inparameters.append (parameters [paramIndex])
        resultValues = wrapCall (parsed.execute, inparameters).getOutputParameters ()
        for resultIndex in xrange (len (outmapping)):
            parameters [outmapping [resultIndex]] = resultValues [resultIndex]
        return parameters
