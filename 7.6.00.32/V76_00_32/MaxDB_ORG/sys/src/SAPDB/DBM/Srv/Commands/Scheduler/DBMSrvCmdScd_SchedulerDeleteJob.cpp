/*!
  \file    DBMSrvCmdScd_SchedulerDeleteJob.cpp
  \author  MarcW
...
DBMSrvCmdScd_SchedulerDeleteJob::DBMSrvCmdScd_SchedulerDeleteJob()
: DBMSrvCmdScd_SchedulerCommand( DBMSrv_Command::KeySchedulerDeleteJob)
{
    m_longHelp=
        "@command scheduler_delete_job This command deletes a scheduled job. It is removed from "
        "the scheduler's job list. This command can be executed no matter whether the scheduler "
        "is running or not."
        "@preconditions The job has no successors. You have the DBM operator authorization "
            "SchedulerMgm or you are the job's owner and have the DBM operator authorization "
            "Scheduling."
        "@syntax scheduler_delete_job <job_id>"
        "@param <job_id> Id of job to be deleted."
        "@reply OK";
}

tcn00_Error DBMSrvCmdScd_SchedulerDeleteJob::runLockProtected(
    ...
    if( !(laSch.setInfoType(DBMSrvShM_LineAccessorScheduler::IT_DeleteJob, errOut) &&
